...
process_no = [16,36,49,64]

for g in process_no:
      st = 'data'+str(g)+'.txt'
      f = open(st, "r")

      send = [[None]*5 for _ in range(7)] #7 rows corresponding to different data points and 5 column each for 5 different runtimes
      pack = [[None]*5 for _ in range(7)]
      datatype = [[None]*5 for _ in range(7)]
